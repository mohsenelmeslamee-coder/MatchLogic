...
function getCachedData(date) {
    ...
    if (cacheAge > 5 * 60 * 1000) {
        console.log(`🗑️ Cache expired for date: ${date}`);
...
// Smart Refresh for Live Matches
function startLiveRefresh() {
    setInterval(() => {
        if (currentPage === 'today' && globalMatchesCache) {
            console.log('🔄 Refreshing live matches...');
            
            // Only update live matches scores
            const liveMatchIds = liveMatches.map(m => m.fixture.id);
            
            // Fetch fresh data for live matches only
            fetch(`/api/matches?date=${getDate('today')}`)
                .then(res => res.json())
                .then(data => {
                    const freshMatches = data.response || [];
                    const updatedLive = freshMatches.filter(m => 
                        liveMatchIds.includes(m.fixture.id)
                    );
                    
                    // Update cached live matches with fresh data
                    if (updatedLive.length > 0) {
                        const cachedMatches = globalMatchesCache.matches || [];
                        updatedLive.forEach(fresh => {
                            const index = cachedMatches.findIndex(m => m.fixture.id === fresh.fixture.id);
                            if (index !== -1) {
                                cachedMatches[index] = fresh;
                            }
                        });
                        
                        cacheMatches(cachedMatches, getDate('today'));
                        updateLiveMatches(cachedMatches);
